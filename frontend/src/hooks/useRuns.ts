...
export function useRuns() {
  ...
  const appendEvent = useCallback((event: StreamRunEvent) => {
    if (event.type === "run_start") {
      const exists = runsRef.current.some((r) => r.run_id === event.run_id);
      if (!exists) {
        const run: Run = {
          run_id: event.run_id,
          status: "running",
        ...
        switch (event.type) {
          case "trace": {
            ...
            return {
              ...
              trace_steps: [
                ...run.trace_steps,
                { step: step.step, message: step.message, timestamp: step.timestamp },
              ],
            };
          }
          case "tool_call": {
            ...
            return {
              ...
              tool_invocations: run.tool_invocations.map((inv) =>
                inv.call_id === tr.call_id
                  ? {
                      ...inv,
                      status: tr.success ? ("success" as const) : ("error" as const),
                      output: tr.output,
  ...
  return { runs, startRun, appendEvent, clearRuns };
}
