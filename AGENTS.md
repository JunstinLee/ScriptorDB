...
**Approval pause semantics (load-bearing):** tools declared `requires_approval=True` surface `DeferredToolRequests`. `runtime/approval/policy.py` auto-approves `LOW_RISK_WRITE_TOOLS` (write_csv, write_file, export_excel, create_table, execute_ddl, write_data, python_sandbox_execute), gates `HIGH_RISK_IMPORT_TOOLS` (import_csv_to_db, import_excel_to_db) on `IMPORT_ROW_THRESHOLD = 100` rows, and always gates `HUMAN_APPROVAL_TOOLS` = {browser_apply_filter}, whose args are editable via `override_args`. `POST /api/sessions/{id}/approve` (`api/routes/approve.py`) calls `orchestrator.signal_approval`, which builds `ToolApproved`/`ToolDenied` and wakes the suspended run — no new stream is opened and the run is never restarted. Runs started by `POST /chat` are owned by `runtime/approval/run_registry` (`ActiveRun` slot) + `runtime/approval/event_bus.RunEventBus`; the owner coroutine `runtime/approval/run_owner.execute_run` gates persistence via `_should_persist` (status `completed`, or `error_type == "site_unavailable"`) and deregisters the slot on the terminal event. Detaching a subscriber (`/chat` or `GET /sessions/{id}/stream`) does **not** affect the run; re-attaching replays buffered events from the `from_index` query cursor. `runtime/approval/policy.py` also defines an `ApprovalPolicy` dataclass that nothing consumes — policy is hardcoded constants.
...
- `runtime/` — `runner/` (lifecycle, translator, events, finalize, errors, takeover_hook), `approval/` (orchestrator, policy, store, pause, persist, resumable, controller, event_bus, run_registry, run_owner), `session_file_store.py`, `session_model.py`, `tool_middleware.py`, `run_tracker.py`, `run_control.py`, `redact.py`, `site_unavailable.py`.
...
- Approval/takeover: `runtime/approval/orchestrator.py`, `runtime/approval/run_registry.py`, `runtime/approval/run_owner.py`, `runtime/approval/event_bus.py`, `runtime/approval/policy.py`, `runtime/approval/resumable.py`, `runtime/runner/takeover_hook.py`, `browser/takeover.py`.
